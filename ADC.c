...
#include "libs/numeros.c" // Arquivos para implementação dos números
#include "libs/definicoes.h" // Arquivos para variáveis globais
...
#define JOYSTICK_X_PIN 26  // Pino ADC conectado ao eixo X do joystick
#define JOYSTICK_Y_PIN 27  // Pino ADC conectado ao eixo Y do joystick
#define BUTTON_PIN 22      // Pino do botão do joystick
#define BUTTON_A_PIN 5     // Pino do botão A
#define LED_GREEN_PIN 11   // Pino do LED Verde
#define LED_RED_PIN 13     // Pino PWM conectado ao LED Vermelho
...
#define DEADZONE 110       // Margem de erro para o centro do joystick
...
void setup_pwm(uint pin) {
    ...
    pwm_set_wrap(slice_num, 255);  // Define o intervalo do PWM para 8 bits (0-255)
...
int main() {
    ...
    int square_x = (SSD1306_WIDTH - SQUARE_SIZE) / 2;
    int square_y = (SSD1306_HEIGHT - SQUARE_SIZE) / 2;
    ...
    while (true) {
        ...
        if (x_value < (CENTER_VALUE - DEADZONE)) {
            brightness_red = (CENTER_VALUE - DEADZONE - x_value) / 8;
        } else if (x_value > (CENTER_VALUE + DEADZONE)) {
            brightness_red = (x_value - (CENTER_VALUE + DEADZONE)) / 8;
        }

        // Mapeia o valor do eixo Y para o brilho do LED Azul
        uint8_t brightness_blue = 0;
        if (y_value < (CENTER_VALUE - DEADZONE)) {
            brightness_blue = (CENTER_VALUE - DEADZONE - y_value) / 8;
        } else if (y_value > (CENTER_VALUE + DEADZONE)) {
            brightness_blue = (y_value - (CENTER_VALUE + DEADZONE)) / 8;
        }

        // Define o ciclo de trabalho do PWM para controlar o brilho dos LEDs
        if (pwm_leds_enabled) {
            set_pwm_duty(LED_RED_PIN, brightness_red);   // LED Vermelho
            set_pwm_duty(LED_BLUE_PIN, brightness_blue); // LED Azul
        } else {
            set_pwm_duty(LED_RED_PIN, 0);  // Desliga o LED Vermelho
            set_pwm_duty(LED_BLUE_PIN, 0); // Desliga o LED Azul
        }
        ...
        if (square_x > SSD1306_WIDTH - SQUARE_SIZE) square_x = SSD1306_WIDTH - SQUARE_SIZE;
        if (square_y < 0) square_y = 0;
        if (square_y > SSD1306_HEIGHT - SQUARE_SIZE) square_y = SSD1306_HEIGHT - SQUARE_SIZE;
